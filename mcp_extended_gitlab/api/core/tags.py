...
def register(mcp: FastMCP):
    ...
    async def get_single_repository_tag(
        project_id: str = Field(description="The ID or URL-encoded path of the project"),
        tag_name: str = Field(description="The name of the tag")) -> Dict[str, Any]:
    ...
    async def create_tag(
        ...
        release_description: Optional[str] = Field(default=None, description="Add release notes to the Git tag and store it in the GitLab database")) -> Dict[str, Any]:
        """Create a new tag."""
        client = await get_gitlab_client()
        data = {
            "tag_name": tag_name,
    ...
    async def delete_tag(
        project_id: str = Field(description="The ID or URL-encoded path of the project"),
        tag_name: str = Field(description="The name of a tag")) -> Dict[str, Any]:
        """Delete a tag."""
        client = await get_gitlab_client()
    ...
    @mcp.tool()
    async def update_tag_release(
        project_id: str = Field(description="The ID or URL-encoded path of the project"),
        tag_name: str = Field(description="The name of a tag"),
        description: str = Field(description="Release notes with markdown support")) -> Dict[str, Any]:
        """Update release notes for a tag."""
        client = await get_gitlab_client()
